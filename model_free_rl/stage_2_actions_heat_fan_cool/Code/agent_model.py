...
class Actor(nn.Module):
    ...
    def get_action_log_probs(self, actions, mu, std):
        ...
        adjustment = torch.mean( torch.log( 1 - self.tanh(actions)**2  + 1e-6 ) , dim = 1) #was sum ()
        
        action_log_probs = torch.mean( normal_dist.log_prob(actions), dim =1 ) - adjustment #was sum ()
