...
def save_train_results(i_episode, train_time, episode_rewards, plot_scores_train, episode_actor_loss, episode_critic_1_loss, episode_critic_2_loss, q_predictions, data_params, model_params, env_attributes , env ):    
    
    metrics_path = model_params["metrics_path"]
    exp_path = model_params["exp_path"]
    points = env_attributes["points"]
    max_episode_length = data_params["time_params"]["max_episode_length"]
...
def save_test_results(i_episode, env, env_attributes, data_params, model_params, actor):
    ...
    start_time_tests = data_params["time_params"]["start_time_tests"]
