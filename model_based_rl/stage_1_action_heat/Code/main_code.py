...
import sys
sys.path.insert(0, r'C:/Users/gauthambekal93/Research/model_based_rl/bestest_hydronic_heat_pump/dynamic_electricity_pricing/Experiment_V31/Code_Results_Models_V10/Code')

import os
os.chdir(r'C:/Users/gauthambekal93/Research/model_based_rl/bestest_hydronic_heat_pump/dynamic_electricity_pricing/Experiment_V31/Code_Results_Models_V10/Code')
...
with open('all_paths.json', 'r') as openfile:  
...
realT_zon_model, dry_bulb_model, reward_model, env_memory_train,completed_initial_env_train =  load_env_model(realT_zon_model, dry_bulb_model, reward_model, env_memory_train, env_model_attributes )
...
for i_episode in range(last_loaded_episode + 1, n_training_episodes+1): 
        ...
        while done==0:
            ...
            #if env_memory_train.memory_size() >20:
            if (len(env_memory_train.states_train) >0) and (len(env_memory_train.states_validation) >0):    
                
                train_neuralnet(reward_model, env_memory_train, exp_path, env_model_attributes) 
                    
                train_neuralnet(realT_zon_model, env_memory_train, exp_path, env_model_attributes ) 
            ...
            if True:
                    ...
                    for _ in range(env_attributes["no_of_updates"]):
                        ...
                        batch_states, batch_actions, batch_rewards, batch_next_states, batch_done = get_train_data( agent_actual_memory , agent_synthetic_memory, env_attributes )
        ...
        if i_episode %5 ==0:
            save_models(i_episode, exp_path, actor,actor_optimizer,critic_1, critic_optimizer_1 , critic_2 , critic_optimizer_2, agent_actual_memory, env_memory_train, agent_synthetic_memory, reward_model, realT_zon_model, env_type)    
   
        
   
'''
checkpoint = { "model_name":reward_model.name,
              'model_state_dicts':  [hypernet.state_dict() for hypernet in reward_model.hypernets ] ,  
                                   'optimizer_state_dicts': [hypernet_optimizer.state_dict() for hypernet_optimizer in reward_model.hypernet_optimizers ] , 
                                    "task_index_loss": { 0: 0.00042575597763061523 } 
                                    }
