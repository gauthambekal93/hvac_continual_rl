...
def initialize_agent(env_attributes):     
     ...
     agent_synthetic_memory = Synthetic_Memory(env_attributes["synthetic_buffer_size"] )
...
def get_train_data( agent_actual_memory , agent_synthetic_memory, env_attributes ):
    
    sample_size = env_attributes["batch_size"]
    
    if ( np.random.rand() >= ( 1 / env_attributes["no_of_updates"] ) ) and (agent_synthetic_memory.memory_size() > 0) :
    ...
    return  states[ :, env_attributes["state_mask"] ], actions [ :,  env_attributes["action_mask"] ] , rewards, next_states[ :,  env_attributes["state_mask"] ], done 
