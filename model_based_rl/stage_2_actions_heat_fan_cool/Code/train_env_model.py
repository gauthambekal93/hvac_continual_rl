...
def validate_model(hypernet, target_model, validation_X, validation_y, task_index, no_of_models):
    ...
    with torch.no_grad():
        ...
        for previous_task_index in range(0, task_index): 
            ...
            weights_old, bias_old = hypernet.generate_weights_bias( previous_task_index, no_of_models)
    ...
    regularization_loss =  beta * regularizer    
    
    return  validation_loss.item(), regularization_loss.item(), final_predictions
...
def train_neuralnet(model, env_memory, exp_path, env_model_attributes ):
    ...
    validation_loss, regularization_loss, validation_predictions = validate_model( model.hypernet, model.target_model, validation_X, validation_y, task_index , no_of_models)
             
    print("Model: " ,model.name, "Hypernet Validation Loss: ", validation_loss, "Hypernet Regularization Loss: ", regularization_loss )
